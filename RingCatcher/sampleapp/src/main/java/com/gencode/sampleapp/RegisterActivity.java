...
//                .getBoolean(com.gencode.ringcatcher.gcm.QuickstartPreferences.SENT_TOKEN_TO_SERVER, false);
//        if (sentToken) {
//            String token = sharedPreferences.getString(com.gencode.ringcatcher.gcm.QuickstartPreferences.TOKEN_ID_FROM_GCM,"");
        ...
        if (isRegistration) {
            AsyncRegisterUser asyncRegisterUser = new AsyncRegisterUser(new IRegisterTask() {
                ...
                public void OnTaskCompleted(RegisterResult registerResult) {
                    ...
                    Log.d(TAG, "AsyncRegisterUser OnTaskCompleted:"+registerResult.toString());
                    if (registerResult != null && !registerResult.equals("")) {
                        //정상등록된 경우 서버전송완료SENT_TOKEN_TO_SERVER=true라는 값저장
                        if (registerResult.getResultCode().equals(ReturnCode.SUCCESS.get())
                                ||registerResult.getResultCode().equals(ReturnCode.UPDATE_OK.get())) {
                            SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(RegisterActivity.this);
                            sharedPreferences.edit().putBoolean(com.gencode.ringcatcher.gcm.QuickstartPreferences.SENT_TOKEN_TO_SERVER, true).apply();
                        }
