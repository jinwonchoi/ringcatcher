...
//    }
     ...
     * 2 음원등록
     */
    public UploadImageResult uploadMessageImage(UploadImageRequest request) {
        ...
        try {
            ...
            String json = multipart(url, body, request.getImageFileName());
            JSONObject jsonRootObject = new JSONObject(json);

            //Get the instance of JSONArray that contains JSONObjects
            result.setResultCode(jsonRootObject.optString("resultCode"));
            result.setResultMsg(jsonRootObject.optString("resultMsg"));
            result.setFileUrl(jsonRootObject.optString("fileUrl"));
    ...
    public RegisterResult registerMessage(RegisterMessageRequest request) {
        RegisterResult result = new RegisterResult();
